# Loading the Sara Blakely Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (72 atoms, 112 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Sara's synthesized worldview, all 72 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Sara's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Sara's thinking.

**Best for:** "I want every decision in this project filtered through Sara's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/sara-blakely/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Sara Blakely" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Sara's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Sara, not generic advice. Try `I have a business idea but I'm scared to start because I have no experience in the industry. What would Sara think?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Sara's expertise.

**Best for:** "I want to consult Sara's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/sara-blakely ~/.brainsfor/brains/sara-blakely
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Sara's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `sara-blakely`.
...
for relevant atoms from Sara Blakely before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/sara-blakely/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `sara-blakely`. Then run `/surprise` — you should get a specific atom from Sara, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Sara guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Sara Blakely brings a specific worldview to your decisions; pair with other brains for productive tension.
