# Loading the Jensen Huang Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (805 atoms, 736 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Jensen's synthesized worldview, all 805 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Jensen's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Jensen's thinking.

**Best for:** "I want every decision in this project filtered through Jensen's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/jensen-huang/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Jensen Huang" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Jensen's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Jensen, not generic advice. Try `I'm a startup founder building AI infrastructure. Should I build on NVIDIA's platform or try to go with an open-source alternative? What would Jensen think about my strategic options?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Jensen's expertise.

**Best for:** "I want to consult Jensen's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/jensen-huang ~/.brainsfor/brains/jensen-huang
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Jensen's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `jensen-huang`.
...
for relevant atoms from Jensen Huang before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/jensen-huang/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ```
   /advise Should I build my own AI infrastructure or rent compute from hyperscalers?
   /debate Is NVIDIA's CUDA moat sustainable, or will open alternatives eventually erode it?
   /predict What happens to the chip industry when every nation demands sovereign AI infrastructure?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `jensen-huang`. Then run `/surprise` — you should get a specific atom from Jensen, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Jensen guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Jensen Huang brings a specific worldview to your decisions; pair with other brains for productive tension.
