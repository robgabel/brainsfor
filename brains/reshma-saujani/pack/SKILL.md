---
name: brain-setup
description: "Load the Reshma Saujani brain. Pack ships 10 unified skills in pack/skills/ — 1 router (/brain) + 8 reasoning modes (/advise, /teach, /debate, /connect, /evolve, /surprise, /coach, /predict) + /board (multi-brain advisory) — that work with this brain and any other installed BrainsFor brain."
---

# Reshma Saujani Brain Pack — Setup & Configuration

Welcome to the **Saujani Brain Pack** from [brainsforfree.com](https://brainsforfree.com). This is a premium knowledge asset extracted from Reshma Saujani's TED talks, book summaries, interviews, and public speeches — a curated collection of her most powerful thinking on bravery, gender equity, girls in tech, imposter syndrome, and the fight for working mothers.

## What You're Getting

- **687 knowledge atoms** — Core insights, frameworks, and strategic principles from 32 sources including TED talks, Google talks, SXSW panels, podcast episodes, commencement addresses, and press profiles
- **668 typed connections** — Relationships between ideas (supports, contradicts, extends, related, inspired_by)
...
- **Cross-referenced topics** — Bravery vs. perfection, Girls Who Code, imposter syndrome, Marshall Plan for Moms, gender equity, tech diversity, working mothers, political courage, and more
...
Your Saujani Brain Pack folder should contain:

```
pack/
  ├── SKILL.md                 ← you are here (brain-setup)
  ├── brain-context.md         ← full knowledge base + usage guide
  ├── brain-atoms.json         ← structured atoms (687 insights)
...
1. **Layer 1 — Quick Reference** (brain-atoms.json)
   - JSON structure of 687 atoms with titles, summaries, connections
...
**Recommended:** Run `/brain reshma-saujani` at the start of each session. The router writes the active brain to `${BRAINSFOR_HOME:-~/.brainsfor}/state/active-brain.txt` and loads `brain-context.md` as session context. After that, the 8 thinking skills (`/advise`, `/teach`, etc.) will use this brain automatically — no need to retype the slug on every call.

### Step 4: Verify Everything Works

Test your setup with the Quick Start prompt below. You should get a thoughtful response grounded in Reshma's thinking.

---

## The 9 Unified Skills

These skills live in `pack/skills/` and are brain-agnostic — they work with Reshma Saujani and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain reshma-saujani`, or override per call with an inline slug like `/advise reshma-saujani <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain reshma-saujani` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*

### 🧠 **1. /advise** — Strategic Counsel
Ask for advice on decisions grounded in Reshma's frameworks. Best for: career moves, company decisions, strategy choices.
*Example: "Should I launch my startup now or wait until I feel more prepared?"*

### 📚 **2. /teach** — Explain Like I'm Learning
Get a clear explanation of a concept as Reshma sees it. Best for: understanding unfamiliar ideas, building mental models.
*Example: "Explain the difference between bravery and perfection to me."*
...
*Example: "Is imposter syndrome a personal problem or a systemic one?"*
...
*Example: "Connect the bravery framework to girls learning to code."*
...
*Example: "How does Reshma's thinking on women's economic equality evolve?"*
...
No answers — just the questions Reshma would ask you. Best for: uncovering blind spots, pressure-testing decisions, team discussions.
*Example: "I'm afraid to ask for a promotion. What questions should I be asking myself?"*
...
*Example: "What happens to the tech industry if we don't close the gender gap?"*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Reshma Saujani installed, `/board` will ask you to add another brain from [brainsforfree.com](https://brainsforfree.com).*
...
*Example: New to the bravery framework? Teach → evolve → coach to build deep understanding and personal application.*
...
*Example: Writing about gender equity in tech? Predict the implications → debate the systemic causes → advise on action.*
...
```
/brain reshma-saujani

/advise "I keep holding back from applying for a leadership role because I don't feel ready. What would Reshma say to me?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise reshma-saujani "I keep holding back from applying for a leadership role because I don't feel ready. What would Reshma say to me?"
```

You should get a thoughtful response that:
- References specific frameworks from the brain
- Weighs tradeoffs from Reshma's perspective
...
→ Use `/advise` with a keyword: `"What does Reshma think about bravery vs. perfection?"` → The brain will pull all relevant atoms.
...
This brain was built exclusively from **freely available, public sources** — Saujani's freely distributed TED talks, public commencement addresses, podcast appearances, YouTube interviews, and widely circulated press profiles.. No transcripts from commercial works (e.g., audiobooks, paid courses) or paywalled essays were used to derive this brain.
...
**You're ready to think with Reshma. Let's go.** 🚀
