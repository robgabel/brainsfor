# Loading the Sun Tzu Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (772 atoms, 906 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Sun's synthesized worldview, all 772 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Sun's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Sun's thinking.

**Best for:** "I want every decision in this project filtered through Sun's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/sun-tzu/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Sun Tzu" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Sun's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Sun, not generic advice. Try `I'm entering a market with a much larger competitor who has more resources. How should I approach this?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Sun's expertise.

**Best for:** "I want to consult Sun's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/sun-tzu ~/.brainsfor/brains/sun-tzu
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Sun's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `sun-tzu`.
...
for relevant atoms from Sun Tzu before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/sun-tzu/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is deception ethical in business strategy, or does it undermine long-term trust?
   /predict What happens when every competitor in a market has access to the same AI-powered intelligence simultaneously?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `sun-tzu`. Then run `/surprise` — you should get a specific atom from Sun, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Sun guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Sun Tzu brings a specific worldview to your decisions; pair with other brains for productive tension.
