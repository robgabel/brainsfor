...
Welcome to the **Tzu Brain Pack** from [brainsforagents.com](https://brainsforagents.com). This is a premium knowledge asset extracted from Sun Tzu's "The Art of War" and attributed historical commentary — a curated collection of his most enduring strategic thinking on warfare, leadership, intelligence, positioning, deception, and the psychology of conflict.
...
These skills live in `pack/skills/` and are brain-agnostic — they work with Sun Tzu and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain sun-tzu`, or override per call with an inline slug like `/advise sun-tzu <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain sun-tzu` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Sun Tzu installed, `/board` will ask you to add another brain from [brainsforagents.com](https://brainsforagents.com).*
...
/advise "I'm entering a market with a much larger competitor who has more resources. How should I approach this?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise sun-tzu "I'm entering a market with a much larger competitor who has more resources. How should I approach this?"
```
...
**You're ready to think with Sun. Let's go.** 🚀
