# Loading the Jesse Pujji Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (653 atoms, 604 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Jesse's synthesized worldview, all 653 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Jesse's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Jesse's thinking.

**Best for:** "I want every decision in this project filtered through Jesse's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/jesse-pujji/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Jesse Pujji" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Jesse's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Jesse, not generic advice. Try `I'm thinking about bootstrapping my startup instead of raising VC. What would Jesse think about my decision?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Jesse's expertise.

**Best for:** "I want to consult Jesse's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/jesse-pujji ~/.brainsfor/brains/jesse-pujji
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Jesse's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `jesse-pujji`.
...
for relevant atoms from Jesse Pujji before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/jesse-pujji/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is bootstrapping always better than raising VC, or does it depend on the business?
   /predict What happens to DTC brands as paid social advertising costs continue to rise?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `jesse-pujji`. Then run `/surprise` — you should get a specific atom from Jesse, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Jesse guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why the cross-brain skills work. Jesse Pujji brings a specific worldview to your decisions; pair with other brains for productive tension.
