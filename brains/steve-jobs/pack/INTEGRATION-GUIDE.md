# Loading the Steve Jobs Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (1029 atoms, 1019 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Steve's synthesized worldview, all 1029 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Steve's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Steve's thinking.

**Best for:** "I want every decision in this project filtered through Steve's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/steve-jobs/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Steve Jobs" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Steve's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Steve, not generic advice. Try `I'm launching a new product and struggling to decide which features to cut. What would Steve think?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Steve's expertise.

**Best for:** "I want to consult Steve's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/steve-jobs ~/.brainsfor/brains/steve-jobs
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Steve's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `steve-jobs`.
...
for relevant atoms from Steve Jobs before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/steve-jobs/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ```
   /advise Should I add more features or strip my product down to its essence?
   /debate Is owning the full hardware-software stack worth the market share you sacrifice?
   /predict What would Steve think about the future of AI-generated interfaces replacing human-designed ones?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `steve-jobs`. Then run `/surprise` — you should get a specific atom from Steve, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Steve guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Steve Jobs brings a specific worldview to your decisions; pair with other brains for productive tension.
