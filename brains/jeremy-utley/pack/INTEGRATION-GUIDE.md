# Loading the Jeremy Utley Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (759 atoms, 2272 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Jeremy's synthesized worldview, all 759 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Jeremy's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Jeremy's thinking.

**Best for:** "I want every decision in this project filtered through Jeremy's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/jeremy-utley/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Jeremy Utley" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Jeremy's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Jeremy, not generic advice. Try `My team feels stuck and our brainstorming sessions keep producing the same ideas. What would Jeremy say is going wrong and how do I fix it?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Jeremy's expertise.

**Best for:** "I want to consult Jeremy's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/jeremy-utley ~/.brainsfor/brains/jeremy-utley
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Jeremy's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `jeremy-utley`.
...
for relevant atoms from Jeremy Utley before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/jeremy-utley/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ```
   /advise Should I give my team more structure or more freedom to be creative?
   /debate Is quantity really more important than quality when it comes to ideas?
   /predict What happens to creative professionals who don't adapt to working with AI?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `jeremy-utley`. Then run `/surprise` — you should get a specific atom from Jeremy, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Jeremy guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Jeremy Utley brings a specific worldview to your decisions; pair with other brains for productive tension.
