# Loading the John Green Brain Pack into Your AI Agent
...
Pick the one that matches how you want John's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by John's thinking.

**Best for:** "I want every decision in this project filtered through John's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/john-green/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: John Green" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by John's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from John, not generic advice. Try `I'm struggling to stay hopeful about the world while also being honest about how broken things are. What would John think?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to John's expertise.

**Best for:** "I want to consult John's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/john-green ~/.brainsfor/brains/john-green
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load John's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `john-green`.
...
for relevant atoms from John Green before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/john-green/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `john-green`. Then run `/surprise` — you should get a specific atom from John, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient John guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. John Green brings a specific worldview to your decisions; pair with other brains for productive tension.
