# Loading the Shiva Rajaraman Brain Pack into Your AI Agent
...
Pick the one that matches how you want Shiva's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Shiva's thinking.

**Best for:** "I want every decision in this project filtered through Shiva's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/shiva-rajaraman/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Shiva Rajaraman" >> CLAUDE.md
   echo "" >> CLAUDE.md
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Shiva's expertise.

**Best for:** "I want to consult Shiva's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/shiva-rajaraman ~/.brainsfor/brains/shiva-rajaraman
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Shiva's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `shiva-rajaraman`.
...
for relevant atoms from Shiva Rajaraman before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/shiva-rajaraman/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `shiva-rajaraman`. Then run `/surprise` — you should get a specific atom from Shiva, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Shiva guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Shiva Rajaraman brings a specific worldview to your decisions; pair with other brains for productive tension.
