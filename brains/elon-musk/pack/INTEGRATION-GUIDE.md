# Loading the Elon Musk Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (836 atoms, 719 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Elon's synthesized worldview, all 836 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Elon's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Elon's thinking.

**Best for:** "I want every decision in this project filtered through Elon's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/elon-musk/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Elon Musk" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Elon's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Elon, not generic advice. Try `I'm building a hardware startup but struggling with manufacturing costs. How would Elon approach this problem?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Elon's expertise.

**Best for:** "I want to consult Elon's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/elon-musk ~/.brainsfor/brains/elon-musk
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Elon's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `elon-musk`.
...
for relevant atoms from Elon Musk before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/elon-musk/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is colonizing Mars worth the investment when we have unsolved problems on Earth?
   /predict What happens to traditional automakers when full self-driving is finally solved?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `elon-musk`. Then run `/surprise` — you should get a specific atom from Elon, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Elon guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Elon Musk brings a specific worldview to your decisions; pair with other brains for productive tension.
