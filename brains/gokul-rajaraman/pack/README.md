...
- **Install this brain pack first** → `/brain gokul-rajaraman` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate gokul-rajaraman paul-graham originality` contrasts two thinkers.
...
**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/gokul-rajaraman/`. Run `/brain gokul-rajaraman` to activate.
