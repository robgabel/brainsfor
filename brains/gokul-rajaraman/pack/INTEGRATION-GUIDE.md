# Loading the Gokul Rajaraman Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (551 atoms, 931 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Gokul's synthesized worldview, all 551 atoms with original quotes and implications, persona rules, skill instructions |
...
1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/gokul-rajaraman/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Gokul Rajaraman" >> CLAUDE.md
   echo "" >> CLAUDE.md
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/gokul-rajaraman ~/.brainsfor/brains/gokul-rajaraman
   ```
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `gokul-rajaraman`.
...
for relevant atoms from Gokul Rajaraman before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/gokul-rajaraman/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `gokul-rajaraman`. Then run `/surprise` — you should get a specific atom from Gokul, not generic advice.
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Gokul Rajaraman brings a specific worldview to your decisions; pair with other brains for productive tension.
