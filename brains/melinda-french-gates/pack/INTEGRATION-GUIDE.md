# Loading the Melinda French Gates Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (439 atoms, 394 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Melinda's synthesized worldview, all 439 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Melinda's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Melinda's thinking.

**Best for:** "I want every decision in this project filtered through Melinda's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/melinda-french-gates/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Melinda French Gates" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Melinda's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Melinda, not generic advice. Try `I want to direct funding toward women's economic empowerment but don't know where to start. What would Melinda think?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Melinda's expertise.

**Best for:** "I want to consult Melinda's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/melinda-french-gates ~/.brainsfor/brains/melinda-french-gates
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Melinda's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `melinda-french-gates`.
...
for relevant atoms from Melinda French Gates before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/melinda-french-gates/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is billionaire philanthropy a force for good or does it concentrate too much power in too few hands?
   /predict What happens to global health outcomes when women control household finances?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `melinda-french-gates`. Then run `/surprise` — you should get a specific atom from Melinda, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Melinda guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Melinda French Gates brings a specific worldview to your decisions; pair with other brains for productive tension.
