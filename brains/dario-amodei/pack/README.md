...
> **353 knowledge atoms** · **502 typed connections** · **9 unified skills (1 router + 8 reasoning modes)**
...
1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/dario-amodei/`
...
You should get a response that cites specific atoms, synthesizes Dario's perspective, and recommends what to do next.
...
pack/
  ...
  ├── brain-atoms.json      ← Structured data (353 atoms, 502 connections)
...
- **Install this brain pack first** → `/brain dario-amodei` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate dario-amodei paul-graham originality` contrasts two thinkers.

## The Knowledge Graph

**353 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**502 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.
...
**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/dario-amodei/`. Run `/brain dario-amodei` to activate.
