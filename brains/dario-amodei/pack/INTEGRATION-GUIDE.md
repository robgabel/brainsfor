# Loading the Dario Amodei Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (353 atoms, 1000 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Dario's synthesized worldview, all 353 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Dario's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Dario's thinking.

**Best for:** "I want every decision in this project filtered through Dario's lens."
...
3. Open Claude Code in your project. The brain is now active — every conversation is informed by Dario's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Dario, not generic advice. Try `I'm a software engineer thinking about whether to work on AI safety. Is it actually tractable, or am I wasting my career on an unsolvable problem?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Dario's expertise.

**Best for:** "I want to consult Dario's brain when relevant, not load it on every message."
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Dario's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `dario-amodei`.
...
for relevant atoms from Dario Amodei before recommending an approach.
```
...
4. Use any thinking skill:
   ```
   /advise Should my AI startup adopt a responsible scaling policy, or will it slow us down competitively?
   /debate Is AI safety research actually slowing down AI capabilities progress?
   /predict What happens if mechanistic interpretability succeeds — what are the second and third-order effects?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `dario-amodei`. Then run `/surprise` — you should get a specific atom from Dario, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Dario guidance + MCP server for deep atom queries when the agent needs specifics |
