---
name: brain-setup
description: "Load the Annie Duke brain. Pack ships 10 unified skills in pack/skills/ — 1 router (/brain) + 8 reasoning modes (/advise, /teach, /debate, /connect, /evolve, /surprise, /coach, /predict) + /board (multi-brain advisory) — that work with this brain and any other installed BrainsFor brain."
---

# Annie Duke Brain Pack — Setup & Configuration

Welcome to the **Duke Brain Pack** from [brainsforagents.com](https://brainsforagents.com). This is a premium knowledge asset extracted from Annie Duke's books, talks, and interviews — a curated collection of her most rigorous thinking on decision-making under uncertainty, quitting strategically, cognitive bias, and probabilistic reasoning.

## What You're Getting

- **1034 knowledge atoms** — Core insights, frameworks, and strategic principles from Three major books (Thinking in Bets, How to Decide, Quit), TED talks, 40+ video interviews and podcast appearances
- **1713 typed connections** — Relationships between ideas (supports, contradicts, extends, related, inspired_by)
...
- **Cross-referenced topics** — Thinking in bets, resulting, decision quality, quitting, cognitive bias, probabilistic thinking, risk, uncertainty, and more
...
Your Duke Brain Pack folder should contain:

```
pack/
  ├── SKILL.md                 ← you are here (brain-setup)
  ├── brain-context.md         ← full knowledge base + usage guide
  ├── brain-atoms.json         ← structured atoms (1034 insights)
...
1. **Layer 1 — Quick Reference** (brain-atoms.json)
   - JSON structure of 1034 atoms with titles, summaries, connections
...
**Recommended:** Run `/brain annie-duke` at the start of each session. The router writes the active brain to `${BRAINSFOR_HOME:-~/.brainsfor}/state/active-brain.txt` and loads `brain-context.md` as session context. After that, the 8 thinking skills (`/advise`, `/teach`, etc.) will use this brain automatically — no need to retype the slug on every call.

### Step 4: Verify Everything Works

Test your setup with the Quick Start prompt below. You should get a thoughtful response grounded in Annie's thinking.

---

## The 9 Unified Skills

These skills live in `pack/skills/` and are brain-agnostic — they work with Annie Duke and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain annie-duke`, or override per call with an inline slug like `/advise annie-duke <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain annie-duke` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*

### 🧠 **1. /advise** — Strategic Counsel
Ask for advice on decisions grounded in Annie's frameworks. Best for: career moves, company decisions, strategy choices.
*Example: "Should I keep investing in a project that hasn't shown results after two years?"*

### 📚 **2. /teach** — Explain Like I'm Learning
Get a clear explanation of a concept as Annie sees it. Best for: understanding unfamiliar ideas, building mental models.
*Example: "Explain 'resulting' and why it leads smart people to make bad decisions."*
...
*Example: "Is quitting ever truly the right choice, or does persistence always win?"*
...
*Example: "Connect poker decision-making to how executives run companies."*
...
*Example: "How does Annie's thinking on quitting evolve from Thinking in Bets to Quit?"*
...
No answers — just the questions Annie would ask you. Best for: uncovering blind spots, pressure-testing decisions, team discussions.
*Example: "I'm doubling down on a strategy that isn't working. What should I be asking myself?"*
...
*Example: "What happens to organizational culture when leaders are rewarded for outcomes rather than decision quality?"*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Annie Duke installed, `/board` will ask you to add another brain from [brainsforagents.com](https://brainsforagents.com).*
...
*Example: New to decision-making? Teach 'resulting' → teach 'thinking in bets' → coach on a real decision to build intuition.*
...
*Example: Writing about cognitive bias? Teach the core concepts → debate whether bias is unavoidable → predict how AI changes human decision-making.*
...
/advise "I'm thinking about whether to stay in a struggling business or cut my losses and quit. What would Annie think?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise annie-duke "I'm thinking about whether to stay in a struggling business or cut my losses and quit. What would Annie think?"
```

You should get a thoughtful response that:
- References specific frameworks from the brain
- Weighs tradeoffs from Annie's perspective
...
→ Use `/advise` with a keyword: `"What does Annie think about resulting?"` → The brain will pull all relevant atoms.
...
This brain was built exclusively from **freely available, public sources** — Annie Duke's publicly available books, TED and Google talks, podcast appearances, YouTube interviews, and widely circulated writing on decision-making, poker, and behavioral psychology.. No transcripts from commercial works (e.g., audiobooks, paid courses) or paywalled essays were used to derive this brain.
...
**You're ready to think with Annie. Let's go.** 🚀
