# Loading the Kara Swisher Brain Pack into Your AI Agent
...
Pick the one that matches how you want Kara's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Kara's thinking.

**Best for:** "I want every decision in this project filtered through Kara's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/kara-swisher/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Kara Swisher" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Kara's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Kara, not generic advice. Try `I'm trying to understand why Kara thinks Big Tech companies have failed democracy. Where would she start?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Kara's expertise.

**Best for:** "I want to consult Kara's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/kara-swisher ~/.brainsfor/brains/kara-swisher
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Kara's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `kara-swisher`.
...
for relevant atoms from Kara Swisher before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/kara-swisher/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `kara-swisher`. Then run `/surprise` — you should get a specific atom from Kara, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Kara guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Kara Swisher brings a specific worldview to your decisions; pair with other brains for productive tension.
