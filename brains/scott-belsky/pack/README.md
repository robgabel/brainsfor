...
> **284 knowledge atoms** · **430 typed connections** · **9 unified skills (1 router + 8 reasoning modes)**
...
1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/scott-belsky/`
...
pack/
  ...
  ├── brain-atoms.json      ← Structured data (284 atoms, 430 connections)
...
- **Install this brain pack first** → `/brain scott-belsky` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate scott-belsky paul-graham originality` contrasts two thinkers.

## The Knowledge Graph

**284 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**430 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.
...
**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/scott-belsky/`. Run `/brain scott-belsky` to activate.
