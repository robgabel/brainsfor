# Loading the Peter Zeihan Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (362 atoms, 1000 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Peter's synthesized worldview, all 362 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Peter's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Peter's thinking.

**Best for:** "I want every decision in this project filtered through Peter's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/peter-zeihan/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Peter Zeihan" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Peter's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Peter, not generic advice. Try `I run a manufacturing business with exposure to Chinese supply chains. What should I be preparing for over the next 10 years?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Peter's expertise.

**Best for:** "I want to consult Peter's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/peter-zeihan ~/.brainsfor/brains/peter-zeihan
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Peter's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `peter-zeihan`.
...
for relevant atoms from Peter Zeihan before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/peter-zeihan/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is China actually collapsing, or is Peter overstating it?
   /predict What happens when the American Navy stops securing global sea lanes?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `peter-zeihan`. Then run `/surprise` — you should get a specific atom from Peter, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Peter guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Peter Zeihan brings a specific worldview to your decisions; pair with other brains for productive tension.
