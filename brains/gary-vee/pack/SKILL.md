---
name: brain-setup
description: "Load the Gary Vaynerchuk brain. Pack ships 10 unified skills in pack/skills/ — 1 router (/brain) + 8 reasoning modes (/advise, /teach, /debate, /connect, /evolve, /surprise, /coach, /predict) + /board (multi-brain advisory) — that work with this brain and any other installed BrainsFor brain."
---
...
Your Vaynerchuk Brain Pack folder should contain:
...
**Recommended:** Run `/brain gary-vee` at the start of each session. The router writes the active brain to `${BRAINSFOR_HOME:-~/.brainsfor}/state/active-brain.txt` and loads `brain-context.md` as session context. After that, the 8 thinking skills (`/advise`, `/teach`, etc.) will use this brain automatically — no need to retype the slug on every call.

### Step 4: Verify Everything Works

Test your setup with the Quick Start prompt below. You should get a thoughtful response grounded in Gary's thinking.

---

## The 9 Unified Skills

These skills live in `pack/skills/` and are brain-agnostic — they work with Gary Vaynerchuk and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain gary-vee`, or override per call with an inline slug like `/advise gary-vee <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain gary-vee` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*

### 🧠 **1. /advise** — Strategic Counsel
Ask for advice on decisions grounded in Gary's frameworks. Best for: career moves, company decisions, strategy choices.
*Example: "Should I quit my job to pursue my side hustle full-time?"*

### 📚 **2. /teach** — Explain Like I'm Learning
Get a clear explanation of a concept as Gary sees it. Best for: understanding unfamiliar ideas, building mental models.
*Example: "Explain 'day trading attention' to me."*
...
*Example: "Is hustle culture toxic or necessary for new entrepreneurs?"*
...
*Example: "Connect self-awareness to business strategy."*
...
No answers — just the questions Gary would ask you. Best for: uncovering blind spots, pressure-testing decisions, team discussions.
*Example: "I'm scared to post on social because I'm worried what people will think. What should I be asking myself?"*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Gary Vaynerchuk installed, `/board` will ask you to add another brain from [brainsforfree.com](https://brainsforfree.com).*
...
/advise "I'm a solo founder trying to build an audience for my software product. I have no budget and I'm nervous about being on camera. What would Gary tell me?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise gary-vee "I'm a solo founder trying to build an audience for my software product. I have no budget and I'm nervous about being on camera. What would Gary tell me?"
```

You should get a thoughtful response that:
- References specific frameworks from the brain
- Weighs tradeoffs from Gary's perspective
...
→ Use `/advise` with a keyword: `"What does Gary think about attention arbitrage?"` → The brain will pull all relevant atoms.
...
**You're ready to think with Gary. Let's go.** 🚀
