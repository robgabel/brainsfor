...
brainsfor/
  ...
  scripts/                           ← SHARED pipeline (brain-agnostic)
    ...
    archive/                         ← completed one-shot migrations/backfills (see archive/README.md). Not pipeline code.
  ...
  website/                           ← **LIVE SITE** — brainsforagents.com (Next.js app on Vercel, auto-deploy from main)
    ...
    app/page.tsx                     ← **homepage = first-mile funnel** (revamped 2026-07-03, spec: ../PRD-homepage-first-mile.md): auto-playing side-by-side HeroDemo → lane switcher → free zip CTA → install strip → beta band. ≤400 visible words by design — new sections go on /brains or /skills, not here
    app/opengraph-image.tsx          ← next/og share card (the side-by-side) — beta traffic arrives from X/HN/Discord links
    components/HeroDemo.tsx          ← canned autoplay demo (data: lib/hero-demos.ts, atom-grounded — verify any copy edit against the pack via brainsfor MCP) + live /api/skill ask + BoardDemo tab; fires @vercel/analytics events (hero_demo_play, lane_switch, download_click, beta_email_click)
    app/skills/page.tsx              ← **/skills = second mile** (redone 2026-07-03): skills organized by JOB, not inventory — Decide (advise/debate/coach, Annie default) / Create (surprise/connect, Belsky) / Learn (teach/evolve/predict, Munger) / Convene (BoardDemo). Same rules as homepage: demo-first, low words, ends on the free-brain ask
    components/SkillJobs.tsx         ← job-tab wrapper around SkillsPlayground; per-skill auto-run seed chips via its `seedsBySkill` prop; `skills_job_switch` analytics event
    middleware.ts                    ← ⚠️ **SITE_PASSWORD gate**: while the `SITE_PASSWORD` env var is set in Vercel prod (set ~May 2026), the ENTIRE site 401s behind Basic auth (user `friend`) — except /brains/* assets, api/skill, api/board, AGENTS.md, llms.txt, opengraph-image. This is the pre-launch gate; "the site looks down" usually means this. Going public = `vercel env rm SITE_PASSWORD production` + redeploy
    lib/                             ← brains.ts (reads brains/index.json for atomCount/connectionCount + per-brain emoji badge, rendered on cards + detail hero); hero-demos.ts (9 canned hero answers w/ atom ids)
    public/brains/                   ← per-brain static assets: brain-atoms.json, brain-context.md, explore.html, SKILL.md, <slug>-brain-pack.zip (zips are gitignored — regenerated by `npm run prebuild` / scripts/sync-brain-assets.mjs; run it once in a fresh checkout or dev downloads 404)
...
- **Phase 2:** Content ingestion — three sub-stages run in sequence:
  ...
  - **Speaker diarization (added 2026-06-20, systemic):** Both extraction paths — `decompose_transcript` (YouTube/audio) and `extract_atoms_from_text` (scraped text) — route interview/podcast sources through `scripts/diarize.py` so `original_quote` is attributed ONLY to the brain subject, never the interviewer/host. Cheapest-first waterfall: **named** (parse `Name:` labels, free, e.g. Mixergy transcripts) → **llm** (fast model splits speakers on unlabeled interview transcripts) → **stt** (AssemblyAI on a podcast `audio_url`, gated on optional `ASSEMBLYAI_API_KEY`) → **none** (passthrough — essays/single-author sources unchanged, fully back-compatible). Same code runs locally and in the remote Action. `INTERVIEW_TYPES` gates the paid LLM/STT tiers so essays never pay for diarization. **Why it exists:** YouTube blocks datacenter IPs (the GitHub Action can't fetch captions), and even reachable transcripts (Happy Scribe etc.) are unlabeled — so without this, interviewer questions leaked into the subject's quotes.
- **Phase 3:** Synthesis (LLM-generated synthesis.md + brain.json synthesis section)
  - **3.5 Synthesis grounding** — `ground-synthesis.py` tags each principle grounded/sourced/attributed/dropped (see SPEC-synthesis-grounding.md).
  - **3.6 Hard Lessons mining (added 2026-06-21)** — `scripts/mine-hard-lessons.py` mines `synthesis.hard_lessons` (mistake → cost → change, with verbatim receipts) from the brain's OWN diarized atoms, enforcing a real-cost rubric. `[]` is the correct output for a sanitized corpus (a QA signal). `--skip-hard-lessons` bypasses. Prompt lives in `auto_build_config.py` (`HARD_LESSONS_PROMPT`). `export-brain.py` renders these into `brain-context.md` from the structured key so the LLM sees them.
...
**Transcript-fetch guardrail (relaxed 2026-06-20):** Phase 2 also watches the "videos discovered but 0 transcripts fetched" case (the YouTube-blocks-CI-IPs failure). This now **only HARD-BLOCKS when the brain is also below the atom floor** — i.e. videos were the expected source AND text sources didn't carry the brain. When text/other sources already clear `MIN_BRAIN_ATOMS` (the common case once diarized interview transcripts are scraped), a transcript failure **degrades to a WARN + continue + ship HIDDEN + flagged** (`progress["transcript_fetch_failed"]`), mirroring the low-voice path — it no longer halts a substantive brain. Previously it hard-blocked any build with ≥`MIN_VIDEOS_EXPECT_TRANSCRIPTS` videos and zero transcripts, even with hundreds of solid text atoms (this is what stalled the Jesse Pujji rebuilds in CI).
...
Config module: `scripts/auto_build_config.py` (shared constants, cost tables, Claude API wrapper with retry logic, prompt templates). **This is the canonical source of truth for all Anthropic model strings used by BrainsFor scripts.** Every Python script in `scripts/` imports `DEFAULT_MODEL`, `SYNTHESIS_MODEL`, and `FAST_MODEL` from here. `website/app/api/skill/route.ts` hardcodes its string separately (TS can't import from Python) with a comment pointing to this file. Current targets (upgraded 2026-07-03): Sonnet 5 (`claude-sonnet-5`) / Opus 4.8 (`claude-opus-4-8`) / Haiku 4.5. Sonnet 5 notes: new tokenizer counts ~30% more tokens for the same text; adaptive thinking is ON when `thinking` is omitted (website routes disable it explicitly); non-default sampling params are rejected.
...
- **⚠️ Corollary — a Quote Verifiability warning can be a checkout artifact, not fabrication.** `audit-brains.py` matches quotes against the LOCAL corpus; a brain whose transcripts were never fetched (or a fresh checkout, since transcripts are gitignored) will flag real transcript quotes as "likely LLM-fabricated". Case: jesse-pujji 2026-07-04 — audit said 175/246 quotes fabricated; `ingest-youtube.py --brain jesse-pujji --from-sources` fetched all 12 transcripts locally and ALL 175 verified (score 84→91, provenance 29%→100%, zero API spend). **Never strip quotes on a provenance warning until transcripts have been fetched locally.**
...
- **Diarization: in-pipeline now, but the shipped 25 predate it.** Going forward, both extraction paths route interview/podcast sources through `scripts/diarize.py` (named → llm → stt → none waterfall) so `original_quote` is attributed only to the subject — see the "Speaker diarization" note in the auto-build pipeline section above. **The 25 brains currently shipped were built before that landed (2026-06-20)**, so their video atoms came from raw single-stream captions: `fetch_transcript` stores segments as `{text, start, duration}` with no speaker field, and most sources are YouTube auto-captions. For their multi-speaker sources (Talks at Google Q&A, Kara Swisher / Oprah interviews, the Greens' podcasts, Attia) an interviewer's or other guest's words can be mis-attributed to the subject. The Phase 2.3b / `ground-synthesis.py` verifiers are **speaker-blind** — they check a quote exists *in the corpus*, and the interviewer's words are in that same corpus, so they cannot catch this; only `diarize.py` (which runs *before* extraction) prevents the leak, which is why pre-diarization packs are worth a re-check or rebuild.
...
**Lessons (2026-07-18, jesse-pujji + scott-belsky attribution audits — elon playbook applied fleet-wide):**
- **jesse-pujji and scott-belsky are now attribution-audited** (ledgers: `brains/<slug>/research/attribution-audit-2026-07-18.json`). Jesse: 31 REATTR + 23 bulk ref-normalizations (352 atoms) applied; belsky: 112 REATTR + 28 bulk normalizations (463 atoms). All reattribution evidence was verbatim-verified against the local corpus (144/144), every applied `source_date` is backed by a public proof URL (YouTube uploadDate metadata / Apple Podcasts / publisher pages), and STRIP proposals passed 2 independent adversarial refuters. Residue (needs-human): elon 4, jesse 33, belsky 27 — documented in the ledgers, untouched in data.
- **Whitelist deliberate ref conventions before adjudicating.** Two prefixes are intentional, not LLM-guessed, and must never be stripped: `Public record: <event>` (depth-fix blown-timeline/self-inversion fact atoms — verified via the human-gated verify pipeline, `proof_ref` holds the authority; corpus absence is BY DESIGN, and a STRIP would clobber `proof_ref`) and `synthesis:<artifact>` (internal QA provenance markers, e.g. jesse's voice-verified handoff). The 2026-07-18 machine pass proposed stripping 5 elon + 1 jesse atoms in these families; all were vetoed on review.
- **x.com / LinkedIn-sourced atoms are structurally unverifiable** — the pipeline never scrapes those platforms, so corpus-absence there is a platform limitation, not evidence of fabrication. Demote to residue, don't strip.
...
**Shipped 2026-06-21** (HANDOFF-brain-qa.md). Tier 3 of the QA regime — judges a brain *from its human's perspective* (would they recognize themselves; is every hard number true; is the mess included; does it break their constitution), the thing structural + behavioral evals can't see.

- **`scripts/verify-claims.py`** — numeric-claims verifier. Scans the brain's assertions (synthesis + `hard_lessons`) for hard figures (revenue/users/raise/exit/date) and checks the corpus for a larger/newer value. Catches the Ampush `$10M→$30M` defect class a string-match audit can't. Writes `evals/numeric-claims-<date>.json`; exit 1 on any high-severity defect (ship-gate signal).
- **`scripts/persona-qa.py`** — 4 independent judges on the `/board` pattern (no judge sees another's verdict): **Kara Swisher** (authenticity), **Charlie Munger** (rigor/anti-slop), **Brené Brown** (coverage of the mess), **Annie Duke (CHAIR**, calibration/judge-of-judges). Graded against the constitution (`first_principles` + `does_not_believe` + `would_not_say` + `hard_lessons`). **Automatic recusal** — no brain judges its own pack or a close peer (alts in `PANEL`/`CLOSE_PEERS`). High-severity numeric defects hard-cap the score. Writes `evals/persona-review-<date>.md` + `persona-qa-<date>.json`. **Runs on Opus 4.8 by default** (`PERSONA_MODEL` in `auto_build_config.py`) — the judgment-heavy panel benefits from Opus; pilot showed every dimension higher + chair confidence medium→high vs Sonnet for ~1.67× cost. `--model claude-sonnet-4-6` halves the cost. The miner + numeric verifier stay on Sonnet (validation-bounded).
- **`scripts/brain-qa.py`** — orchestrator. Composite scorecard across all tiers + the **ship-gate** (`hidden → live` only when persona-QA ≥ 70, **zero high-severity numeric defects**, voice ≥ 0.60). `--brain <slug>` runs missing tiers (~$0.46 — Sonnet numeric + Opus panel); `--all` is a fleet table, **cached by default** (cost guard) — `--run` to spend (~$10 for the 21 live brains). Exit 0 = gate clear.
- **`scripts/publish-qa-scores.py`** — publishes QA scores to the website (shipped 2026-07-04). Reads the latest `evals/persona-qa-*.json` + `numeric-claims-*.json` per brain plus audit voice, and writes a `qa` block onto each entry in `brains/index.json` (`score`, `dimensions{authenticity,rigor,coverage,calibration}`, `voice`, `numeric_defects`, `confidence`, `as_of`). The website reads it from there: catalog cards show a color-tiered "QA <score>" pill (`website/components/QaScore.tsx` — green ≥75 / indigo 60-74 / amber below), detail pages a full "Quality score" breakdown with dimension bars + methodology expandable. Brains without a persona-qa artifact get their `qa` key removed (no stale scores). **Run after every QA refresh** so the site stays in sync; `--dry-run` prints the table without writing. Policy: publish ALL scores raw, including low ones — transparency is the brand.
- **`promote-brain.py --persona-gate`** — opt-in (default off); refuses a v2-over-live promotion unless the staging brain passed its persona-QA.
...
- **`LAUNCH.md`** — **THE canonical launch/marketing plan** (2026-07-04, updated 07-06): P0/P1/P2 checklist + PRD-1..8 (legal pages, dashboard fix, install-path honesty, SEO metadata, go-public runbook, beta recruitment, launch-week content, **launch-lineup/QA-transparency decision — pending Rob**). Goal: 10 beta users. PRD-1..4 merged + deployed behind the gate. `PRD-site-overhaul.md` was deleted as superseded; GTM sections in `business-plan.md`/`IMPROVEMENTS.md` now point here.
