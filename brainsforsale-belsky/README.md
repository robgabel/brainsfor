# Scott Belsky Brain Pack

> **284 knowledge atoms** · **161 typed connections** · **10 thinking skills**
> From [brainsforsale.com](https://brainsforsale.com)

## What This Is

A structured knowledge graph of Scott Belsky's thinking, extracted from 77 editions of the Implications newsletter. Designed to be loaded as context into any LLM — Claude, ChatGPT, Cursor, or any AI tool that accepts system prompts.

This isn't a chatbot or a persona. It's a **thinking partner** grounded in Scott's actual ideas, frameworks, and worldview.

## Quick Start

1. Load `brain-context.md` into your AI tool's context window
2. Run any of the 10 thinking skills:

```
/advise "Should I raise my Series A or stay bootstrapped?"
```

You should get a response that cites specific atoms, synthesizes Scott's perspective, and recommends what to do next.

## What's Inside

```
brainsforsale-belsky/
  ├── SKILL.md              ← Setup guide + skill reference (start here)
  ├── brain-context.md      ← Full knowledge base (load this as context)
  ├── brain-atoms.json      ← Structured data (284 atoms, 161 connections)
  ├── README.md             ← You are here
  └── skills/
      ├── advise.md         ← Strategic counsel
      ├── teach.md          ← Learn concepts in Scott's voice
      ├── debate.md         ← Steel-man counterarguments
      ├── connect.md        ← Find unexpected bridges
      ├── evolve.md         ← Trace how ideas developed
      ├── apply.md          ← Turn frameworks into action
      ├── mashup.md         ← Synthesize ideas into new theses
      ├── brainfight.md     ← Pit ideas head-to-head
      ├── deep-dive.md      ← Exhaustive topic analysis
      └── surprise.md       ← Random high-quality insight
```

## The Knowledge Graph

**284 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**161 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.

**16 topic clusters** — Superhumanity, originality, AI agents, organizational design, longevity, emergence, decision-making, scaling.

**Date range:** May 2014 — April 2026

## How to Use

**For Claude Code / Cowork:** Skills load automatically. Just invoke `/advise`, `/teach`, etc.

**For Cursor:** Add `brain-context.md` to your `.cursor/rules` or project instructions.

**For ChatGPT / Claude.ai:** Paste `brain-context.md` as your first message or system prompt.

**For any other tool:** Load `brain-context.md` as context at the start of your session.

## Skill Workflows

Chain skills for deeper results:

- **Decide:** `/advise` → `/debate` → `/apply`
- **Learn:** `/teach` → `/deep-dive` → `/evolve`
- **Create:** `/surprise` → `/connect` → `/mashup`
- **Research:** `/deep-dive` → `/mashup` → `/brainfight`
- **Daily ritual:** `/surprise` each morning

## Cross-Brain Features

Install multiple brain packs and they auto-discover each other:

- `/mashup` synthesizes across brains
- `/brainfight` compares thinkers head-to-head
- No configuration needed — just install and go

## About Scott Belsky

Scott Belsky is a legendary entrepreneur and strategic thinker who has shaped product and organizational thinking across the tech industry. His "Implications" newsletter is where he shares his latest thinking on originality, organizational design, AI, and the future of work.

## Support

- Full setup guide: `SKILL.md`
- Skill reference: `skills/README.md`
- Website: [brainsforsale.com](https://brainsforsale.com)

---

**Load a genius into your AI.**
