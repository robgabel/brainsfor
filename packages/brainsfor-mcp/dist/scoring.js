...
export function tokenize(text) {
    return (text.toLowerCase().match(/[a-z][a-z'-]+/g) ?? []).filter((t) => t.length > 2 && !STOPWORDS.has(t));
}
/**
 ...
 */
export function scoreAtomsForQuery(atoms, query, k) {
    const qTokens = tokenize(query);
    if (qTokens.length === 0)
        return [];
    const qSet = new Set(qTokens);
    const scored = atoms.map((a) => {
        const topicTokens = new Set((a.topics ?? []).flatMap((t) => tokenize(String(t))));
