...
export function registerGetSynthesis(server: McpServer, brainsDir: string) {
  server.registerTool(
    "get_synthesis",
    {
      title: "Get Brain Synthesis",
      description:
        "Get a thinker's intellectual operating system: first principles, thinking patterns, contrarian positions, what they don't believe, and what they'd never say. This is the foundation for reasoning like this thinker. Always call this first before querying atoms.",
      inputSchema: z
        .object({
    ...
    async ({ brain_slug }) => {
      ...
      if (!entry) {
        return {
          content: [{ type: "text" as const, text: `Brain "${brain_slug}" not found. Use list_brains to see installed brains.` }],
          isError: true,
        };
      }
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error loading brain "${brain_slug}": ${e}` }],
          isError: true,
        };
      }
    }
