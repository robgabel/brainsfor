...
export function registerGetConnections(server: McpServer, brainsDir: string) {
  server.registerTool(
    "get_connections",
    {
      title: "Get Atom Connections",
      description:
        "Get typed connections for a specific atom: what supports it, contradicts it, extends it, or relates to it. Optionally traverse depth=2 for second-order connections. Use this to trace reasoning chains through the knowledge graph.",
      inputSchema: z
        .object({
    ...
    async ({ brain_slug, atom_id, relationship, depth }) => {
      ...
      if (!entry) {
        return {
          content: [{ type: "text" as const, text: `Brain "${brain_slug}" not found.` }],
          isError: true,
        };
      }
      ...
      try {
        ...
        if (!sourceAtom) {
          return {
            content: [{ type: "text" as const, text: `Atom "${atom_id}" not found in brain "${brain_slug}".` }],
            isError: true,
          };
        }
        ...
        const connectionSet = new Map<
          string,
          { atom_id: string; relationship: string; confidence: number; direction: string }
        >();
        ...
        let secondOrder: typeof connections = [];
        if (depth === 2) {
          ...
          for (const firstConn of connections) {
            ...
            for (const subConn of firstAtom.connections) {
              if (firstOrderIds.has(subConn.target_id)) continue;
              if (relationship && subConn.relationship !== relationship) continue;
        ...
        return {
          content: [{ type: "text" as const, text: `Error getting connections: ${e}` }],
          isError: true,
        };
      }
    }
