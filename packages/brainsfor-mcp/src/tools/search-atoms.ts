...
export function registerSearchAtoms(server: McpServer, brainsDir: string) {
  server.registerTool(
    "search_atoms",
    {
      ...
      description:
        "Full-text search across atom content, original quotes, and implications. Scores results by relevance and confidence. Use this to find atoms that topic-based query_atoms might miss.",
      inputSchema: z
        .object({
    ...
    async ({ brain_slug, query, limit }) => {
      ...
      try {
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { brain: brain_slug, query, count: results.length, atoms: results },
                null,
                2
              ),
            },
        ...
        return {
          content: [{ type: "text" as const, text: `Error searching atoms: ${e}` }],
          isError: true,
        };
      }
    }
