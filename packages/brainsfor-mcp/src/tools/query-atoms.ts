...
export function registerQueryAtoms(server: McpServer, brainsDir: string) {
  server.registerTool(
    "query_atoms",
    {
      title: "Query Brain Atoms by Topic",
      description:
        "Retrieve atoms from a brain pack filtered by topic tags, cluster, or confidence. Uses pre-built topic index for fast O(1) lookup. Returns atoms with content, original quotes, implications, and confidence tiers.",
      inputSchema: z
        .object({
    ...
    async ({ brain_slug, topics, cluster, confidence_min, limit }) => {
      ...
      try {
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { brain: brain_slug, count: results.length, atoms: results },
                null,
                2
              ),
            },
        ...
        return {
          content: [{ type: "text" as const, text: `Error querying atoms: ${e}` }],
          isError: true,
        };
      }
    }
