# Loading the Yann LeCun Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (609 atoms, 616 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Yann's synthesized worldview, all 609 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Yann's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Yann's thinking.

**Best for:** "I want every decision in this project filtered through Yann's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/yann-lecun/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Yann LeCun" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Yann's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Yann, not generic advice. Try `I'm trying to understand why LeCun thinks large language models won't be enough to reach human-level AI. What's his core argument?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Yann's expertise.

**Best for:** "I want to consult Yann's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/yann-lecun ~/.brainsfor/brains/yann-lecun
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Yann's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `yann-lecun`.
...
for relevant atoms from Yann LeCun before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/yann-lecun/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is the current path of scaling LLMs sufficient to achieve AGI?
   /predict What happens to AI safety and autonomy if we successfully build agents with world models?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `yann-lecun`. Then run `/surprise` — you should get a specific atom from Yann, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Yann guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Yann LeCun brings a specific worldview to your decisions; pair with other brains for productive tension.
