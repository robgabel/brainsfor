...
> **222 knowledge atoms** · **366 typed connections** · **9 unified skills (1 router + 8 reasoning modes)**
...
1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/hank-green/`
...
pack/
  ...
  ├── brain-atoms.json      ← Structured data (222 atoms, 366 connections)
...
- **Install this brain pack first** → `/brain hank-green` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate hank-green paul-graham originality` contrasts two thinkers.

## The Knowledge Graph

**222 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**366 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.
...
**Date range:** 2007 — April 2026

## How to Use

**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/hank-green/`. Run `/brain hank-green` to activate.
