---
name: brain-setup
description: "Load the Brené Brown brain. Pack ships 10 unified skills in pack/skills/ — 1 router (/brain) + 8 reasoning modes (/advise, /teach, /debate, /connect, /evolve, /surprise, /coach, /predict) + /board (multi-brain advisory) — that work with this brain and any other installed BrainsFor brain."
---

# Brené Brown Brain Pack — Setup & Configuration

Welcome to the **Brown Brain Pack** from [brainsforagents.com](https://brainsforagents.com). This is a premium knowledge asset extracted from Brené Brown's Research on vulnerability, courage, shame, and empathy — a curated collection of her groundbreaking research on vulnerability, courage, shame resilience, and wholehearted living.
...
Your Brown Brain Pack folder should contain:
...
**Recommended:** Run `/brain brene-brown` at the start of each session. The router writes the active brain to `${BRAINSFOR_HOME:-~/.brainsfor}/state/active-brain.txt` and loads `brain-context.md` as session context. After that, the 8 thinking skills (`/advise`, `/teach`, etc.) will use this brain automatically — no need to retype the slug on every call.

### Step 4: Verify Everything Works

Test your setup with the Quick Start prompt below. You should get a thoughtful response grounded in Brené's thinking.

---

## The 9 Unified Skills

These skills live in `pack/skills/` and are brain-agnostic — they work with Brené Brown and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain brene-brown`, or override per call with an inline slug like `/advise brene-brown <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain brene-brown` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Brené Brown installed, `/board` will ask you to add another brain from [brainsforagents.com](https://brainsforagents.com).*
...
/advise "I'm struggling with perfectionism and it's affecting my relationships. What would Brené say about this?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise brene-brown "I'm struggling with perfectionism and it's affecting my relationships. What would Brené say about this?"
```

You should get a thoughtful response that:
- References specific frameworks from the brain
- Weighs tradeoffs from Brené's perspective
...
→ Use `/advise` with a keyword: `"What does Brené think about vulnerability?"` → The brain will pull all relevant atoms.
...
**You're ready to think with Brené. Let's go.** 🚀
