# Loading the Brené Brown Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (1195 atoms, 2513 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Brené's synthesized worldview, all 1195 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Brené's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Brené's thinking.

**Best for:** "I want every decision in this project filtered through Brené's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/brene-brown/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Brené Brown" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Brené's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Brené, not generic advice. Try `I'm struggling with perfectionism and it's affecting my relationships. What would Brené say about this?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Brené's expertise.

**Best for:** "I want to consult Brené's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/brene-brown ~/.brainsfor/brains/brene-brown
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Brené's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `brene-brown`.
...
for relevant atoms from Brené Brown before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/brene-brown/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is radical vulnerability always appropriate in professional settings?
   /predict What happens to a culture where leaders refuse to model vulnerability?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `brene-brown`. Then run `/surprise` — you should get a specific atom from Brené, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Brené guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Brené Brown brings a specific worldview to your decisions; pair with other brains for productive tension.
