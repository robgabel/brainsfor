# Loading the Peter Attia Brain Pack into Your AI Agent
...
1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/peter-attia/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Peter Attia" >> CLAUDE.md
   echo "" >> CLAUDE.md
...
**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Peter, not generic advice. Try `I'm 45 and want to maximize my chances of being healthy at 85. Where do I start?`.
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/peter-attia ~/.brainsfor/brains/peter-attia
   ```
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `peter-attia`.
...
for relevant atoms from Peter Attia before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/peter-attia/pack/skills/* ~/.claude/skills/
   ```
...
**Verify it works:** Run `/brain list` — you should see `peter-attia`. Then run `/surprise` — you should get a specific atom from Peter, not generic advice.
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Peter Attia brings a specific worldview to your decisions; pair with other brains for productive tension.
