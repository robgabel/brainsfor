# Loading the Gary Vaynerchuk Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (929 atoms, 848 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Gary's synthesized worldview, all 929 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Gary's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Gary's thinking.

**Best for:** "I want every decision in this project filtered through Gary's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/gary-vee/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Gary Vaynerchuk" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Gary's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Gary, not generic advice. Try `I'm a solo founder trying to build an audience for my software product. I have no budget and I'm nervous about being on camera. What would Gary tell me?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Gary's expertise.

**Best for:** "I want to consult Gary's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/gary-vee ~/.brainsfor/brains/gary-vee
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Gary's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `gary-vee`.
...
for relevant atoms from Gary Vaynerchuk before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/gary-vee/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ```
   /advise Should I quit my job to pursue my side hustle full-time?
   /debate Is hustle culture toxic or necessary for new entrepreneurs?
   /predict What happens to creator brands if AI makes content production essentially free?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `gary-vee`. Then run `/surprise` — you should get a specific atom from Gary, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Gary guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Gary Vaynerchuk brings a specific worldview to your decisions; pair with other brains for productive tension.
