...
> **246 knowledge atoms** · **505 typed connections** · **9 unified skills (1 router + 8 reasoning modes)**
...
1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/gary-vee/`
...
You should get a response that cites specific atoms, synthesizes Gary's perspective, and recommends what to do next.
...
pack/
  ...
  ├── brain-atoms.json      ← Structured data (246 atoms, 505 connections)
...
- **Install this brain pack first** → `/brain gary-vee` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate gary-vee paul-graham originality` contrasts two thinkers.

## The Knowledge Graph

**246 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**505 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.
...
**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/gary-vee/`. Run `/brain gary-vee` to activate.
