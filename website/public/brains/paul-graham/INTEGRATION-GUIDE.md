# Loading the Paul Graham Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (1135 atoms, 1187 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Paul's synthesized worldview, all 1135 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Paul's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Paul's thinking.

**Best for:** "I want every decision in this project filtered through Paul's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/paul-graham/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Paul Graham" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Paul's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Paul, not generic advice. Try `I'm a developer with a side project getting traction. Should I quit my job and go full-time on it?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Paul's expertise.

**Best for:** "I want to consult Paul's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/paul-graham ~/.brainsfor/brains/paul-graham
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Paul's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `paul-graham`.
...
for relevant atoms from Paul Graham before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/paul-graham/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is it better to build something a lot of people kind of want, or something a few people really want?
   /predict What happens to programming as a profession as AI gets better at writing code?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `paul-graham`. Then run `/surprise` — you should get a specific atom from Paul, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Paul guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Paul Graham brings a specific worldview to your decisions; pair with other brains for productive tension.
