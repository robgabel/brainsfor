# Loading the Reshma Saujani Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (687 atoms, 668 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Reshma's synthesized worldview, all 687 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Reshma's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Reshma's thinking.

**Best for:** "I want every decision in this project filtered through Reshma's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/reshma-saujani/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Reshma Saujani" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Reshma's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Reshma, not generic advice. Try `I keep holding back from applying for a leadership role because I don't feel ready. What would Reshma say to me?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Reshma's expertise.

**Best for:** "I want to consult Reshma's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/reshma-saujani ~/.brainsfor/brains/reshma-saujani
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Reshma's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `reshma-saujani`.
...
for relevant atoms from Reshma Saujani before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/reshma-saujani/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ```
   /advise Should I launch my startup now or wait until I feel more prepared?
   /debate Is imposter syndrome a personal problem or a systemic one?
   /predict What happens to the tech industry if we don't close the gender gap?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `reshma-saujani`. Then run `/surprise` — you should get a specific atom from Reshma, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Reshma guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Reshma Saujani brings a specific worldview to your decisions; pair with other brains for productive tension.
