# Reshma Saujani Brain Pack

> **687 knowledge atoms** · **668 typed connections** · **10 unified skills (1 router + 8 reasoning modes + a board of advisors)**
> From [brainsforfree.com](https://brainsforfree.com)

## What This Is

A structured knowledge graph of Reshma Saujani's thinking, extracted from 32 sources including TED talks, Google talks, SXSW panels, podcast episodes, commencement addresses, and press profiles. Designed to be loaded as context into any LLM — Claude, ChatGPT, Cursor, or any AI tool that accepts system prompts.

This isn't a chatbot or a persona. It's a **thinking partner** grounded in Reshma's actual ideas, frameworks, and worldview.

## Quick Start

1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/reshma-saujani/`
...
You should get a response that cites specific atoms, synthesizes Reshma's perspective, and recommends what to do next.
...
pack/
  ...
  ├── brain-atoms.json      ← Structured data (687 atoms, 668 connections)
...
- **Install this brain pack first** → `/brain reshma-saujani` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate reshma-saujani paul-graham originality` contrasts two thinkers; `/board <question>` convenes every installed brain at once (needs 2+).

## The Knowledge Graph

**687 atoms** — Each atom is a self-contained insight with topic tags, source date, confidence score, and source URL.

**668 connections** — Typed relationships between atoms: supports, contradicts, extends, related, inspired_by. These let skills trace reasoning chains and surface productive tensions.

**16 topic clusters** — Bravery vs. perfection, Girls Who Code, imposter syndrome, Marshall Plan for Moms, gender equity, tech diversity, working mothers, political courage.

**Date range:** 2010 — 2023

## How to Use

**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/reshma-saujani/`. Run `/brain reshma-saujani` to activate.
