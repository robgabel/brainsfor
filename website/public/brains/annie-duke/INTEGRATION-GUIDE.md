# Loading the Annie Duke Brain Pack into Your AI Agent
...
pack/
  ...
  brain-atoms.json         ~1MB     Structured data (1034 atoms, 1713 typed connections)
...
|------|-------------|-----------------|
| `brain-context.md` | Your AI agent | Annie's synthesized worldview, all 1034 atoms with original quotes and implications, persona rules, skill instructions |
...
Pick the one that matches how you want Annie's brain to work in your agent.

### Path A: CLAUDE.md Injection (Simplest — Always-On)

Paste `brain-context.md` into your project's `CLAUDE.md`. Every conversation in that project gets shaped by Annie's thinking.

**Best for:** "I want every decision in this project filtered through Annie's lens."

**Steps:**

1. Copy this pack's contents into your project:
   ```bash
   cp -r path/to/annie-duke/pack ./brain-pack
   ```

2. Append the brain context to your project's `CLAUDE.md`:
   ```bash
   echo "" >> CLAUDE.md
   echo "# Brain: Annie Duke" >> CLAUDE.md
   echo "" >> CLAUDE.md
   cat brain-pack/brain-context.md >> CLAUDE.md
   ```

3. Open Claude Code in your project. The brain is now active — every conversation is informed by Annie's frameworks, first principles, and guardrails.

**Verify it works:** Ask your agent a strategy question and check that the response cites specific ideas from Annie, not generic advice. Try `I'm thinking about whether to stay in a struggling business or cut my losses and quit. What would Annie think?`.
...
Register the BrainsFor MCP server. Your agent queries specific atoms when facing decisions relevant to Annie's expertise.

**Best for:** "I want to consult Annie's brain when relevant, not load it on every message."
...
1. Install this brain pack to the standard location:
   ...
   cp -r path/to/annie-duke ~/.brainsfor/brains/annie-duke
   ```
...
4. Restart Claude Code. The MCP server auto-starts and exposes 6 tools:
   ...
   | `get_synthesis` | Load Annie's intellectual OS (~4KB) |
...
**Verify it works:** In Claude Code, ask "what brains do I have installed?" — the agent should call `list_brains` and show `annie-duke`.
...
for relevant atoms from Annie Duke before recommending an approach.
```
...
1. Copy the skills to your Claude Code skills directory:
   ```bash
   cp -r path/to/annie-duke/pack/skills/* ~/.claude/skills/
   ```
...
4. Use any thinking skill:
   ...
   /debate Is quitting ever truly the right choice, or does persistence always win?
   /predict What happens to organizational culture when leaders are rewarded for outcomes rather than decision quality?
   /surprise
   ```

**Verify it works:** Run `/brain list` — you should see `annie-duke`. Then run `/surprise` — you should get a specific atom from Annie, not generic advice.
...
| Combo | Effect |
|-------|--------|
| **A + B** | brain-context.md in CLAUDE.md for ambient Annie guidance + MCP server for deep atom queries when the agent needs specifics |
...
This is why installing multiple brains gives you genuinely different perspectives — and why `/board` (multi-brain advisory) and `/debate <brain-a> <brain-b>` (cross-brain) work. Annie Duke brings a specific worldview to your decisions; pair with other brains for productive tension.
