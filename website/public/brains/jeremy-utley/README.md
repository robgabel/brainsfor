...
> **911 knowledge atoms** · **961 typed connections** · **10 unified skills (1 router + 8 reasoning modes + a board of advisors)**
...
1. Install the brain pack into `${BRAINSFOR_HOME:-~/.brainsfor}/brains/jeremy-utley/`
...
You should get a response that cites specific atoms, synthesizes Jeremy's perspective, and recommends what to do next.
...
- **Install this brain pack first** → `/brain jeremy-utley` activates it, then `/advise` / `/teach` / `/surprise` all work.
...
- **Cross-brain mode** — `/debate jeremy-utley paul-graham originality` contrasts two thinkers; `/board <question>` convenes every installed brain at once (needs 2+).
...
**For Claude Code / Cowork:** The skills in `pack/skills/` load automatically once the pack is installed to `${BRAINSFOR_HOME:-~/.brainsfor}/brains/jeremy-utley/`. Run `/brain jeremy-utley` to activate.
