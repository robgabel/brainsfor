---
name: brain-setup
description: "Load the Jeremy Utley brain. Pack ships 10 unified skills in pack/skills/ — 1 router (/brain) + 8 reasoning modes (/advise, /teach, /debate, /connect, /evolve, /surprise, /coach, /predict) + /board (multi-brain advisory) — that work with this brain and any other installed BrainsFor brain."
---

# Jeremy Utley Brain Pack — Setup & Configuration

Welcome to the **Utley Brain Pack** from [brainsforagents.com](https://brainsforagents.com). This is a premium knowledge asset extracted from Jeremy Utley's Stanford d.school courses, Ideaflow book, public talks, essays, podcast interviews, and YouTube lectures — a curated collection of his most practical thinking on ideaflow, AI as creative teammate, creative confidence, innovation habits, and building organizations that generate better ideas.
...
Your Utley Brain Pack folder should contain:
...
**Recommended:** Run `/brain jeremy-utley` at the start of each session. The router writes the active brain to `${BRAINSFOR_HOME:-~/.brainsfor}/state/active-brain.txt` and loads `brain-context.md` as session context. After that, the 8 thinking skills (`/advise`, `/teach`, etc.) will use this brain automatically — no need to retype the slug on every call. The brain also stays in voice for the entire session: every turn responds as the thinker until you run `/brain clear`.

### Step 4: Verify Everything Works

Test your setup with the Quick Start prompt below. You should get a thoughtful response grounded in Jeremy's thinking.

---

## The 9 Unified Skills

These skills live in `pack/skills/` and are brain-agnostic — they work with Jeremy Utley and with any other BrainsFor brain installed under `${BRAINSFOR_HOME:-~/.brainsfor}/brains/`. Set the active brain with `/brain jeremy-utley`, or override per call with an inline slug like `/advise jeremy-utley <question>`.

### 🎛️ **0. /brain** — Router
Set, show, list, or clear the active brain for the session.
*Usage: `/brain jeremy-utley` activates, `/brain` shows current, `/brain list` shows all installed, `/brain clear` unsets.*

### 🧠 **1. /advise** — Strategic Counsel
Ask for advice on decisions grounded in Jeremy's frameworks. Best for: career moves, company decisions, strategy choices.
*Example: "Should I give my team more structure or more freedom to be creative?"*
...
*Example: "Is quantity really more important than quality when it comes to ideas?"*
...
*Example: "Connect idea quotas to building a culture of psychological safety."*
...
No answers — just the questions Jeremy would ask you. Best for: uncovering blind spots, pressure-testing decisions, team discussions.
*Example: "I want to become more creative but I feel like I'm not a naturally creative person. What should I be asking myself?"*
...
*Example: "What happens to creative professionals who don't adapt to working with AI?"*
...
*Requires 2+ brains installed, plus a host that can run parallel sub-agents (Claude Code's Agent tool or Cowork). No MCP server needed — each board member reads its brain's local pack. With only Jeremy Utley installed, `/board` will ask you to add another brain from [brainsforagents.com](https://brainsforagents.com).*
...
*Example: New to ideaflow? Teach → evolve → coach to build practical creative habits.*
...
/advise "My team feels stuck and our brainstorming sessions keep producing the same ideas. What would Jeremy say is going wrong and how do I fix it?"
```

Or skip the activation step and use an inline slug for a one-off:

```
/advise jeremy-utley "My team feels stuck and our brainstorming sessions keep producing the same ideas. What would Jeremy say is going wrong and how do I fix it?"
```

You should get a thoughtful response that:
- References specific frameworks from the brain
- Weighs tradeoffs from Jeremy's perspective
...
→ Use `/advise` with a keyword: `"What does Jeremy think about ideaflow?"` → The brain will pull all relevant atoms.
...
**You're ready to think with Jeremy. Let's go.** 🚀
